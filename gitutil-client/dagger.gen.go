...
func (r *Gitutil) DefaultBranch(ctx context.Context, gitBase *Container, repo string) (string, error) {
	if r.defaultBranch != nil {
		return *r.defaultBranch, nil
	}
	q := r.q.Select("defaultBranch")
	q = q.Arg("gitBase", gitBase)
	q = q.Arg("repo", repo)
...
func (r *Gitutil) LatestSemverTag(ctx context.Context, gitBase *Container, repo string, prefix string) (string, error) {
	if r.latestSemverTag != nil {
		return *r.latestSemverTag, nil
	}
	q := r.q.Select("latestSemverTag")
	q = q.Arg("gitBase", gitBase)
	q = q.Arg("repo", repo)
...
func (r *Client) Gitutil() *Gitutil {
	q := r.q.Select("gitutil")
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "GitutilClient":
		...
		case "Latest":
			var err error
			var parent GitutilClient
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				fmt.Println(err.Error())
				os.Exit(2)
			}
			return (*GitutilClient).Latest(&parent, ctx)
		case "":
			var err error
			var typeDefBytes []byte = []byte("{\"asObject\":{\"functions\":[{\"name\":\"Latest\",\"returnType\":{\"kind\":\"StringKind\"}}],\"name\":\"GitutilClient\"},\"kind\":\"ObjectKind\"}")
